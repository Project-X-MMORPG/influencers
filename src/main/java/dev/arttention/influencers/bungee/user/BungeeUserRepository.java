...
public class BungeeUserRepository extends UserRepository {
        ...
        while (resultSet.next()) {
          ...
          if (services.has("youtube")) {
            ...
            if (channelVerified) {
              ...
              channel = new Channel(channelName, channelSubscribers, channelVideos,
                  channelTotalViews, channelLastVideos);
              currentRank = this.plugin.getConfiguration()
                  .getYouTubeRank(youtubeJson.getString("current-rank"));
            }
            serviceMap.put(ServiceType.YOUTUBE,
                new YouTubeService(lastCacheTime, name, channelId, channelVerificationKey,
                    channelVerified, channel, currentRank));
          } else {
            // TODO: Add loading Twitch service.
          }
          this.add(new User(UUID.randomUUID(), name, EnumSet.noneOf(ServiceType.class), serviceMap,
              Instant.now()));
        }
      } catch (Exception e) {
  ...
  public boolean save(User user) {
    ...
    for (ServiceType serviceType : ServiceType.values()) {
      blacklistedServicesObject.addProperty(serviceType.getId(),
          user.hasServiceBlacklisted(serviceType));
    }
    ...
    for (Entry<ServiceType, Service> entry : user.getServices().entrySet()) {
      ...
      if (serviceType == ServiceType.YOUTUBE) {
        ...
        serviceObject.addProperty("last-cache-time",
            youTubeService.getLastCacheTime().toEpochMilli());
        serviceObject.addProperty("channel-id", youTubeService.getChannelId());
        serviceObject.addProperty("channel-verification-key",
            youTubeService.getChannelVerificationKey());
    ...
    mySQL.executeQuery("SELECT * FROM `youtubers` WHERE `unique_id` = ?;", resultSet -> {
      if (resultSet == null) {
        mySQL.executeUpdate(
            "INSERT INTO `youtubers`(`unique_id`, `name`, `services`) VALUES (?,?,?)",
            user.getUniqueId().toString(), user.getName(), jsonObject.toString());
        return;
      }

      mySQL.executeUpdate(
          "UPDATE `youtubers` SET `name` = ?, `services` = ? WHERE `unique_id` = ?;",
          user.getName(), jsonObject.toString(), user.getUniqueId().toString());
  ...
  public void updateService(User user, ServiceType serviceType) {
    ...
    if (serviceType == ServiceType.YOUTUBE && this.plugin.getConfiguration().isYouTubeEnabled()) {
      ...
      if (service.getLastCacheTime()
          .plusMillis(this.plugin.getConfiguration().getYouTubeCacheTime().toEpochMilli())
          ...
          for (String newRankCommand : newRank.getCommands()) {
            ProxyServer.getInstance().getPluginManager()
                .dispatchCommand(ProxyServer.getInstance().getConsole(),
                    newRankCommand.replace("{player}", user.getName()));
          }
        }
      }
      ...
      if (service.getLastCacheTime()
          .plusMillis(this.plugin.getConfiguration().getTwitchCacheTime().toEpochMilli())
          .compareTo(Instant.now()) >= 0) {
        return;
      }
      String result = service.update(this.plugin);
